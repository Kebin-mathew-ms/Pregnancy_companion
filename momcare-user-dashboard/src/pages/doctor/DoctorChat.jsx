...
import { MessageSquare, Send, Search, User, Droplets, Activity } from 'lucide-react'

const accent = 'hsl(340, 75%, 45%)'
const accentLight = 'hsl(340, 75%, 93%)'

export default function DoctorChat({ user }) {
  ...
  const fetchChatHistory = () => {
    const docLid = user?.login_id || user?.Login_id || 4
    if (!selectedPatient) return
    const targetLid = selectedPatient.Login_id || selectedPatient.login_id
    if (!targetLid) return

    setLoadingMessages(true)
    fetch(`/api/chatdetail?sender_id=${docLid}&receiver_id=${targetLid}`)
      .then(r => r.json())
      .then(d => {
        const msgs = d.data || []
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      const res = await fetch(`/api/chat?sender_id=${docLid}&receiver_id=${targetLid}&details=${encodeURIComponent(textToSend)}`)
  ...
  return (
    <div style={{ padding: '30px', display: 'flex', flexDirection: 'column', gap: '20px', fontFamily: 'Outfit, sans-serif', height: 'calc(100vh - 100px)', minHeight: '600px' }}>
      ...
      {/* Header */}
      <div>
        <h2 style={{ fontSize: '28px', fontWeight: 800, marginBottom: '4px' }}>Patient Consultation Chat</h2>
        <p style={{ color: 'var(--text-secondary)', fontSize: '14px' }}>
          Direct clinical messaging and guidance with pregnant patients.
        </p>
      </div>
                  ...
                  >
                    ...
                    <div style={{ minWidth: 0, flex: 1 }}>
                      ...
                      <div style={{ fontSize: '12px', color: 'var(--text-secondary)', display: 'flex', gap: '8px', marginTop: '2px' }}>
                        <span>BP: {p.Blood_Pressure || '120/80'}</span>
                        <span>•</span>
          ...
          {selectedPatient ? (
            <>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  ...
                  <div>
                    ...
                    <span style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>
                      Age: {selectedPatient.Age || 'N/A'} yrs | Blood Group: {selectedPatient.Blood_Group || 'O+'} | BP: {selectedPatient.Blood_Pressure || '120/80'}
                    </span>
                  </div>
                </div>
              ...
              <div style={{ flex: 1, minHeight: 0, padding: '24px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '14px' }}>
                {loadingMessages ? (
                  ...
                  <div style={{ textAlign: 'center', margin: 'auto', color: 'var(--text-secondary)' }}>
                    ...
                    <div style={{ fontSize: '15px', fontWeight: 600 }}>No consultation messages yet</div>
                    <div style={{ fontSize: '13px' }}>Send a clinical note or message to {selectedPatient.Full_Name} below.</div>
                  </div>
                ) : (
                  messages.map((m, idx) => {
                    const isMe = String(m.sender_id) === String(user?.login_id || user?.Login_id || 4)
                    return (
                      <div
                        key={idx}
                        style={{
                      ...
                      >
                        <div style={{
                          ...
                          background: isMe ? `linear-gradient(135deg, ${accent}, hsl(340,85%,60%))` : 'white',
              ...
              >
                <input
                  className="form-control"
                  placeholder={`Type consultation message for ${selectedPatient.Full_Name}...`}
                  value={newMessage}
